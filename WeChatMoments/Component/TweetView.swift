import UIKit
protocol DidTapedFavourite : AnyObject {
    func userDidTappedFavourite(tweet:Tweet)
}
...
class TweetView: UITableViewHeaderFooterView {
    ...
    weak var delegate : DidTapedFavourite?
...
//            self.addImageViews(imageURLs)
    ...
    private func setupViews() {
        ...
        self.favouriteButton.titleLabel?.font = UIFont.systemFont(ofSize: Constants.FONT_SIZE_CONTENT)
        self.favouriteButton.setTitle("❤︎", for: .normal)
        self.favouriteButton.setTitleColor(UIColor.red, for: .normal)
        self.favouriteButton.addTarget(self, action: #selector(didPressedFavourite(_:)), for: .touchUpInside)
        self.contentView.addSubview(self.favouriteButton)
        
        self.favouriteButton.translatesAutoresizingMaskIntoConstraints = false

        NSLayoutConstraint.activate([
            self.favouriteButton.heightAnchor.constraint(equalToConstant: 40),
            self.favouriteButton.widthAnchor.constraint(equalToConstant: 40),
            self.favouriteButton.rightAnchor.constraint(equalTo: self.contentView.rightAnchor, constant: Constants.TWEET_SENDER_LEFT_OFFSET),
            self.favouriteButton.topAnchor.constraint(equalTo: self.contentView.topAnchor, constant: 8)
        ])
    ...
    @objc func didPressedFavourite(_ sender:UIButton){
        if self.tweet != nil {
            delegate?.userDidTappedFavourite(tweet: self.tweet)
        }        
    }
}
