...
extension MomentViewController : DidTapedFavourite{
    func userDidTappedFavourite(tweet: Tweet) {
        if let tweetContent = tweet.content{
            UserDefaults.standard.addToFavourites(tweet: tweetContent)
        }        
        if let viewController = FavouriteListTableViewController.getFavouriteViewController(){
